...
function loadSampleProjects() {
    ...
    if (true) {
        const sampleProjects = [
            ...
            {
                id: 'model-rocket-aerodynamics',
                title: 'Model Rocket Aerodynamics Analysis w/ FlowNet and Launch Testing',
                briefDescription: 'Aerodynamics analysis and launch testing of model rockets using FlowNet CFD simulations and field testing.',
                fullDescription: '<strong>What I did:</strong> I launched and analyzed multiple model rockets while conducting comprehensive aerodynamic analysis using my FlowNet platform. This project combined hands-on rocket testing with computational fluid dynamics to validate theoretical predictions against real-world flight data. I documented the complete process from CAD modeling through launch testing, using CFD simulations to analyze rocket aerodynamics.<br><br><strong>How I did it:</strong> I obtained a model rocket kit that came with two rockets (Amazon rocket and a Crossfire rocket) and made precise CAD models based on my measurements of their geometry. I then ran aerodynamic simulations using FlowNet to predict drag coefficients, stability margins, and flow patterns around the rocket bodies. I conducted multiple launch tests with different motor configurations: the Amazon rocket with a B4-4 motor, and the Crossfire rocket with an A8-3 motor and separately with a B4-4 motor.<br><br>Using FlowNet, I ran CFD simulations to compute critical aerodynamic parameters including drag force and drag coefficient for the rocket geometry:<br><br><div class="inline-image-container"><img src="rocketProject/crossfireFlowNet.png" alt="FlowNet Aerodynamic Simulation Data" class="inline-project-image"><p class="image-subtitle">FlowNet simulation computing drag force and drag coefficient for Crossfire rocket geometry</p></div><br><br>The FlowNet simulations accurately captured the varying drag coefficients and drag forces during the burn phase of the launch vs the post-burn phase. This change occurs due to a low pressure zone created behind the rocket from the output of fast-moving propellant gases, which significantly alters the aerodynamic profile compared to the coast phase.<br><br>To predict maximum altitude and maximum speed, I coded equations in C++ for calculating burn altitude, coast altitude, and velocity throughout the flight. This analysis loads in a thrust curve for the specific rocket motor used and time steps through the burn time, calculating the rocket\'s path at each moment. This approach provides a much more accurate and finer-tuned result than simply using average thrust values. The drag force and drag coefficient values from FlowNet simulations are incorporated at each time step to compute expected performance metrics before each launch:<br><br><div class="flownet-horizontal-container"><div class="flownet-horizontal-row"><div class="flownet-media-item"><img src="rocketProject/modelRocketCode1.png" alt="C++ Code Part 1" class="code-image-horizontal"></div><div class="flownet-media-item"><img src="rocketProject/modelRocketCode2.png" alt="C++ Code Part 2" class="code-image-horizontal"></div><div class="flownet-media-item"><img src="rocketProject/modelRocketCode3.png" alt="C++ Code Part 3" class="code-image-horizontal"></div></div><p class="image-subtitle" style="text-align: center; margin-top: 1rem;">C++ analysis code for computing maximum altitude and maximum speed using thrust curves and time-stepping through burn phase with drag force and drag coefficient from FlowNet simulation</p></div><br><br><div class="large-videos-container"><div class="large-video-section"><h4 class="large-video-header">Amazon Rocket with B4-4 Motor</h4><div class="large-video-combo"><div class="large-main-video"><video class="large-featured-video" autoplay muted loop playsinline controls><source src="rocketProject/amazonLaunch.mp4" type="video/mp4">Your browser does not support the video tag.</video></div><div class="large-supplementary-image"><img src="rocketProject/amazonAlt.jpg" alt="Amazon Rocket Altitude Graph" class="large-combo-image"><img src="rocketProject/amazonSpeed.jpg" alt="Amazon Rocket Speed Graph" class="large-combo-image"></div></div><div class="video-analysis"><p>The Amazon rocket with B4-4 motor demonstrated stable flight characteristics with excellent recovery and clean parachute deployment. Pre-launch aerodynamic analysis using FlowNet helped predict fin geometry effects and body tube aerodynamics. The launch showed predicted flight patterns matching closely with CFD simulations.</p><div class="performance-table"><h5>Performance Comparison</h5><table><thead><tr><th>Metric</th><th>Predicted</th><th>Actual</th><th>Accuracy</th></tr></thead><tbody><tr><td>Max Altitude</td><td>45 m</td><td>37 m</td><td>82%</td></tr><tr><td>Max Speed</td><td>24 m/s</td><td>22 m/s</td><td>92%</td></tr></tbody></table></div></div></div><div class="large-video-section"><h4 class="large-video-header">Crossfire Rocket with A8-3 Motor - First Launch</h4><div class="large-video-combo"><div class="large-main-video"><video class="large-featured-video" autoplay muted loop playsinline controls><source src="rocketProject/crossA8.mp4" type="video/mp4">Your browser does not support the video tag.</video></div><div class="large-supplementary-image"><img src="rocketProject/crossA8Alt.jpg" alt="Crossfire A8-3 Altitude Graph" class="large-combo-image"><img src="rocketProject/crossA8Speed.jpg" alt="Crossfire A8-3 Speed Graph" class="large-combo-image"></div></div><div class="video-analysis"><p>The Crossfire rocket with A8-3 motor featured a compact design which allowed it to fly higher on the same motor when compared to the amazon. FlowNet simulations predicted the aerodynamic behavior throughout the flight envelope. Unfortunately, the parachute failed to deploy cleanly on this test flight, providing valuable data on recovery system reliability.</p><div class="performance-table"><h5>Performance Comparison</h5><table><thead><tr><th>Metric</th><th>Predicted</th><th>Actual</th><th>Accuracy</th></tr></thead><tbody><tr><td>Max Altitude</td><td>57 m</td><td>48 m</td><td>84%</td></tr><tr><td>Max Speed</td><td>43 m/s</td><td>42 m/s</td><td>98%</td></tr></tbody></table></div></div></div><div class="large-video-section"><h4 class="large-video-header">Crossfire Rocket with B4-4 Motor - Second Launch</h4><div class="large-video-combo"><div class="large-main-video"><video class="large-featured-video" autoplay muted loop playsinline controls><source src="rocketProject/crossB4.mp4" type="video/mp4">Your browser does not support the video tag.</video></div><div class="large-supplementary-image"><img src="rocketProject/crossB4Alt.jpg" alt="Crossfire B4-4 Altitude Graph" class="large-combo-image"><img src="rocketProject/crossB4Speed.jpg" alt="Crossfire B4-4 Speed Graph" class="large-combo-image"></div></div><div class="video-analysis"><p>The second Crossfire launch used a B4-4 motor for higher performance. Similar to the first launch, the parachute again failed to deploy cleanly.</p><div class="performance-table"><h5>Performance Comparison</h5><table><thead><tr><th>Metric</th><th>Predicted</th><th>Actual</th><th>Accuracy</th></tr></thead><tbody><tr><td>Max Altitude</td><td>125 m</td><td>97 m</td><td>78%</td></tr><tr><td>Max Speed</td><td>52 m/s</td><td>48 m/s</td><td>92%</td></tr></tbody></table></div></div></div></div>',
                whyInteresting: '<strong>Why it\'s interesting:</strong> This project bridges the gap between computational analysis and real-world validation. By using FlowNet to predict aerodynamic performance and then verifying those predictions through actual launches, I demonstrated the practical value of FlowNet as a CFD tool for amateur rocketry.',
                category: 'cfd',
                technologies: 'FlowNet, CFD, Model Rocketry, Aerodynamics, C++',
                previewImage: 'rocketProject/amazonPrep.JPG',
                mediaUrl: ['rocketProject/amazonLaunch.mp4', 'rocketProject/crossA8.mp4', 'rocketProject/crossB4.mp4'],
                mediaType: 'video',
                supplementaryImage: ['rocketProject/amazonAlt.jpg', 'rocketProject/amazonSpeed.jpg', 'rocketProject/crossA8Alt.jpg', 'rocketProject/crossA8Speed.jpg', 'rocketProject/crossB4Alt.jpg', 'rocketProject/crossB4Speed.jpg'],
                layoutType: 'rocket-launch-videos',
                isFeatured: true,
                isSample: true
            },
            {
            ...
            },
            {
                id: 'cpp-ray-tracer',
                title: 'C++ Ray Tracer',
                briefDescription: 'Ray tracer developed from base C++ with recursive reflections, implementing ray collision algorithms and mathematical rendering using linear algebra.',
            ...
            },
            {
                ...
                briefDescription: 'Analysis of urban heat island effects using drone-collected atmospheric temperature and dew point data across five different surface types.',
            ...
            },
            {
                ...
                briefDescription: 'Intelligent collision detection system for highway cable barriers using Python force simulations, Arduino programming, communications for emergency response.',
...
function createProjectItem(project) {
    ...
    if (project.previewImage) {
        ...
        } else if (project.id === 'model-rocket-aerodynamics') {
            thumbnailClass += ' rocket-thumbnail';
        }
...
document.addEventListener('DOMContentLoaded', function() {
    const animateElements = document.querySelectorAll('.expertise-item, .project-card');
